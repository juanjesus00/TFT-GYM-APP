    ...
    if(isMenuVisible){
        Dialog(onDismissRequest = onDismiss) {
            Column(
                ...
                GetMenuVideoButton(
                    onClick = {
                        if(isCurrentUserLogged && isEmailVerified) navigationActions.navigateToRoutineGeneratorPage() else  Toast.makeText(context, "Tienes que iniciar sesión y verificar la cuenta para usar esta función", Toast.LENGTH_LONG).show()
                    },
                    text = "new_routine",
                    enable = true)
            }
