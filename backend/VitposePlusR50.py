...
class repetition:
    ...
    def calculate_repetition(self, elbowsAngels, extensionAngle, contractionAngle, current_frame):
        if self.stateCounting == False:
            valid_angles = [a for a in elbowsAngels if a is not None]
            if len(valid_angles) == 2 and np.mean(valid_angles) > extensionAngle:
        ...
        if self.stateCounting:
            valid_angles = [a for a in elbowsAngels if a is not None]
            if len(valid_angles) == 2 and np.mean(valid_angles) < contractionAngle:
                self.stateCounting = False
...
MIN_AREA = 5000          # Área mínima en píxeles para considerar como persona válida
...
def process_video(input_path, output_path=None, target_fps=30, display_scale=100, progress_path = None, exercise = None):
    ...
    while cap.isOpened():
        ...
        for pose_result in pose_results[0]:
            ...
            reps_duration.append(rep_data['seconds'])
            total_reps = repsCounter.reps 
            texto = [
    ...
    return {
        "reps": total_reps,
        "reps_durations": repsCounter.repsDurations,
        "mean_speed" : np.mean(repsCounter.repsDurations)
    }
...
if __name__ == "__main__":
    video_path = "press_banca4.mp4"
    output_path = "testVideosPersonDetector/press_banca_frontal_rtdetr_r50.mp4"
    #video_path = "frames\\groundTruth\\PressBanca\\frame_00570.jpg"
    #output_path = "GroundTruth\\PressBanca-vitpose-huge\\frame_00570_vitpose_huge.jpg"
    #json_output = "GroundTruth\\PressBanca-vitpose-huge\\coco_frame_00570_vitpose_huge.json"
